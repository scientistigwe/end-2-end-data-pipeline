...
import { setError as setGlobalError, setLoading } from '../store/decisionsSlice';
...
interface DecisionContextType {
  ...
  // Decision Actions
  loadDecisions: (pipelineId: string, filters?: DecisionFilters) => Promise<void>;
  ...
  addVote: (decisionId: string, vote: DecisionVote, comment?: string) => Promise<void>;
...
export const DecisionContext = createContext<DecisionContextType | undefined>(undefined);
...
export const DecisionProvider: React.FC<DecisionProviderProps> = ({ children }) => {
  ...
  // Decision Actions
  const loadDecisions = useCallback(async (pipelineId: string, filters?: DecisionFilters) => {
  ...
  const makeDecision = useCallback(async (decisionId: string, optionId: string, comment?: string) => {
    dispatch(setLoading(true));
    setIsLoading(true);
    try {
      const updatedDecision = await DecisionService.makeDecision(decisionId, optionId, comment);
  ...
  const deferDecision = useCallback(async (decisionId: string, reason: string, deferUntil: string) => {
    dispatch(setLoading(true));
    setIsLoading(true);
    try {
      const updatedDecision = await DecisionService.deferDecision(decisionId, reason, deferUntil);
      setDecisions(prev => prev.map(d => d.id === decisionId ? updatedDecision : d));
      clearError();
    } catch (err) {
  ...
  const addVote = useCallback(async (decisionId: string, vote: DecisionVote, comment?: string) => {
    dispatch(setLoading(true));
    setIsLoading(true);
    try {
