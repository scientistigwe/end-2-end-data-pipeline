...
export const RecommendationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Actions
  const loadRecommendations = useCallback(async (pipelineId: string) => {
    setIsLoading(true);
  ...
  const applyRecommendation = useCallback(async (recommendationId: string, actionId: string) => {
    setIsLoading(true);
  ...
  const dismissRecommendation = useCallback(async (recommendationId: string, reason?: string) => {
    try {
      await RecommendationService.dismissRecommendation(recommendationId, reason);
