...
class InsightRun(BaseModel):
    ...
    configuration = Column(JSONB)  # Analysis configuration
    business_goals = Column(JSONB)  # Associated business goals
    parameters = Column(JSONB)  # Analysis parameters
    ...
    impact_score = Column(Float)  # Overall impact score
...
class Insight(BaseModel):
    ...
    priority = Column(Enum('low', 'medium', 'high', 'critical', name='insight_priority'))
    ...
    visualization = Column(JSONB)  # Visualization config
...
class InsightPattern(BaseModel):
    ...
    seasonality = Column(JSONB)  # Seasonality information
    trend = Column(JSONB)  # Trend information
    ...
    # Context
    conditions = Column(JSONB)  # Conditions when pattern occurs
    exceptions = Column(JSONB)  # Exceptions to the pattern
...
class InsightCorrelation(BaseModel):
    ...
    # Analysis details
    time_window = Column(String(50))  # e.g., 'daily', 'weekly'
    lag = Column(Integer)  # Time lag in analysis
...
class InsightAction(BaseModel):
    ...
    description = Column(Text, nullable=False)
    priority = Column(Enum('low', 'medium', 'high', 'critical', name='action_priority'))
...
class InsightGoalMapping(BaseModel):
    ...
    relevance_score = Column(Float)  # How relevant is this insight to the goal
    impact_score = Column(Float)  # Measured impact on goal
    contribution_type = Column(String(100))  # How this insight contributes
...
class InsightFeedback(BaseModel):
    ...
    context = Column(JSONB)  # Additional context
