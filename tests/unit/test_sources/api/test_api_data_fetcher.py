...
from data_pipeline.source.api.data_fetcher import APIDataFetcher
from data_pipeline.source.api.models import APIResponse, APIConfig
...
def test_test_connection_success(fetcher):
    with patch('data_pipeline.source.api.validator.APIValidator.validate_url') as mock_validate_url:
        mock_validate_url.return_value = (True, None)
        with patch('data_pipeline.source.api.api_client.APIClient.fetch_data') as mock_fetch_data:
...
def test_fetch_data_success(fetcher):
    with patch('data_pipeline.source.api.validator.APIValidator.validate_url') as mock_validate_url:
        mock_validate_url.return_value = (True, None)
        with patch('data_pipeline.source.api.validator.APIValidator.validate_headers') as mock_validate_headers:
