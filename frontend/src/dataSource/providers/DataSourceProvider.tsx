...
export const DataSourceProvider: React.FC<DataSourceProviderProps> = ({ children }) => {
  ...
  const [selectedSource, setSelectedSourceState] = useState<DataSourceConfig | null>(null);
  ...
  // Actions
  const loadDataSources = useCallback(async (filters?: DataSourceFilters) => {
    setIsLoading(true);
  ...
  const updateDataSourceConfig = useCallback(async (
    id: string,
  ...
  const deleteDataSource = useCallback(async (id: string) => {
    ...
    try {
      ...
      setDataSourcesState(prev => prev.filter(source => source.id !== id));
  ...
  const previewDataSource = useCallback(async (
    id: string,
  ...
  const setFilters = useCallback((newFilters: DataSourceFilters) => {
    setFiltersState(newFilters);
    dispatch(setStoreFilters(newFilters));
  }, [dispatch]);

  const selectDataSource = useCallback((source: DataSourceConfig | null) => {
